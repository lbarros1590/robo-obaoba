...
async function obaobaSync(email, password, userId) {
  ...
  try {
    ...
    if (todosOsProdutosExtraidos.length > 0) {
      // ... (lógica de salvar no banco de dados continua a mesma)
    }
...
async function resolveCaptcha(siteKey, pageUrl, apiKey) {
    ...
    while (true) {
        ...
        const check = await axios.get(`http://www.2captcha.com/res.php`, { params: { key: apiKey, action: 'get', id: requestId, json: 1 } });
