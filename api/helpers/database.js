async function synchronizeDatabase(supabase, userId, scrapedProducts) {
    ...
    const { data: existingProductsData, error: fetchError } = await supabase
      .from('products')
      .select('variant_id, purchase_price, stock, is_active, photo, title, base_name, variation_details')
      .eq('user_id', userId);
    ...
    const allScrapedVariantIds = new Set(scrapedProducts.map(p => p.variant_id));
    ...
    for (const scrapedProduct of scrapedProducts) {
      ...
      if (!existingProduct || 
          (existingProduct.purchase_price !== scrapedProduct.purchase_price ||
           ...
           existingProduct.title !== scrapedProduct.title)
      ) {
